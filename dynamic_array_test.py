...
class TestDynamicArray(unittest.TestCase):
    ...
    def test_filter(self, a: List[int]) -> None:
        ...
        self.assertEqual(to_list(filter_(arr,
                                         lambda x: (x % 3 == 0) if x is not None else False)), result)
        result = list(gt_filter(lambda x: x % 3 != 0, a))
        self.assertEqual(to_list(filter_(arr,
                                         lambda x: (x % 3 != 0) if x is not None else False)), result)
    ...
    def test_map(self, a: List[int]) -> None:
        ...
        self.assertEqual(to_list(map_(arr1,
                                      lambda x: x ** 3 if x is not None else x)), result)
